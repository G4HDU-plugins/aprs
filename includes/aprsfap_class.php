...
// #);
class aprsfap{
    private $version = '1.20';
    private $debug = 0;
    private $result_messages;
    private $mice_messagetypes;
    private $rethash;
    private $dstsymbol;
    private $knot_to_kmh ; # nautical miles per hour to kilometers per hour
    private $mph_to_kmh ; # miles per hour to kilometers per hour
    private $kmh_to_ms ; # kilometers per hour to meters per second
    private $mph_to_ms; # miles per hour to meters per second
    private $hinch_to_mm ; # hundredths of an inch to millimeters
...
'Q1' => '\\{' 'Q2' => '\\|' 'Q3' => '\\}' 'Q4' => '\\~'
    ...
    /*
		...
		=cut
*/
    private function result_messages($message){
        return $this->result_messages[$message];
    }
    // sub result_messages()
    // {
    ...
    // from the module
    private function _a_err($rethash, $errcode, $val){
        $this->rethash['resultcode'] = $errcode;
        $this->rethash['resultmsg'] = in_array($errcode, $result_messages)? $this->result_messages[$errcode]:$errcode;
        if (isset($val)){ // check if isset is the correct thing
            $this->rethash['resultmsg'] .= ': ' . $val ;
        }
        if ($debug){
            // make this a log to file
    ...
    // }
    // }
    function _a_warn($rethash, $errcode, $val){
        array_push($this->rethash['warncodes'] , $errcode);
        if ($debug > 0){
            // log a warning
            // warn "Ham::APRS::FAP WARNING $errcode: " . (defined $result_messages{$errcode}? $result_messages{$errcode} : $errcode). (defined $val ? ": $val" : '') . "\n";
        }
    }
    // sub _a_warn($$;$)
    // {
...
=cut
*/
    function mice_mbits_to_message($bits){
        return isset($this->mice_messagetypes[$bits])?$this->mice_messagetypes[$bits]:false;
    }
...
*/
    ...
    private function _gettime($tstamp = null){
        if (is_null($tstamp) || empty($tstamp)){
            // a time stamp has not been passwed in
            $tstamp = time();
        }
        $timestring = gmdate('Y-m-d H:i:s', $tstamp) . 'UTC';
        return $timestring;
    }
    /*
    sub _gettime{
	   ...
	   */
    private function distance($lon0, $lat0, $lon1, $lat1){
        // decimal to radian
        $lon0 = deg2rad($lon0);
        $lon1 = deg2rad($lon1);
        $lat0 = deg2rad($lat0);
        ...
        $distance = $c * 6366.71; # in kilometers
        return $distance;
    }
    ...
    is not too exact, but good enough for us . = back = cut
    ...
    sub distance($$$$){
        ...
        return $distance;
    } = over = item direction($lon0, $lat0, $lon1, $lat1)
*/
    private function direction($lon0, $lat0, $lon1, $lat1){
        // decimal to radian
        $lon0 = deg2rad($lon0);
        $lon1 = deg2rad($lon1);
        $lat0 = deg2rad($lat0);
    ...
    in decimal degrees, north and east positive . = back = cut
    ...
    sub direction($$$$){
        ...
        return rad2deg($direction);
    } = over = item count_digihops($header)
	   */
    private function count_digihops($header){
        // my $header = shift _;
        ...
        $hops = undef;
        if (1
            // $header = ~ / ^([A - Z0 - 9 - ] +)\ > ([A - Z0 - 9 - ] +)$ / o
            ){
            // check the callsigns for validity
            $retval = $this->check_ax25_call($1);
            if ($retval === false){
                if ($debug > 0){
    ...
    //	$header = ~ / ^([A - Z0 - 9 - ] +)\ > ([A - Z0 - 9 - ] +), ([A - Z0 - 9, * - ] +)$ / o
    	){
            $retval = check_ax25_call($1);
            if (not(defined($retval))$retval === false){
                if ($debug > 0){
                  //  warn "count_digihops: invalid source callsign ($1)\n";
                }
                return - 1;
    ...
    return it . Returns - 1 in case of error .
	   ...
	   */
	private function _parse_timestamp($options, $stamp){
    ...
    sub _parse_timestamp($$){
        ...
        elsif ($stamptype eq 'z' || $stamptype eq '/'){
	...
	private function _get_posresolution($arg){
	   return $this->knot_to_kmh * ($arg <= - 2 ? 600 : 1000) * 10 * * (- 1 * $arg);
	}
	...
	*/
    ...
    sub _nmea_getlatlon($$$){
        ...
        if ($value = ~ / ^\s * (\d{
            ...
            $rh->{
                ...
                'posresolution'} = _get_posresolution(length($3));
        }else{
        ...
        if ($dstcallsign = ~ / ^(GPS|SPC)([A - Z0 - 9]{
            ...
            if ($sublength == 3){
                if ($type eq 'C' || $type eq 'E'){
                    ...
                    if (($type eq 'O' || $type eq 'A' || $type eq 'N' || $type eq 'D' || $type eq 'S' || $type eq 'Q') && $overlay = ~ / ^[A - Z0 - 9]$ / o){
                        if (defined($dstsymbol{
                                    $dsttype}
                                )){
                            $code = substr($dstsymbol{
                                    $dsttype}, 1, 1);
                            return ($overlay, $code);
                        }else{
                            return undef;
    ...
    sub _nmea_to_decimal($$$$$){
        ...
        $body = ~ s / \ * [0 - 9A - F]{
            2}
        $// ; # remove checksum from body first
        ...
        if ($nmeafields[0] eq 'GPRMC'){
            ...
            if ($year >= 2038 || $year < 1970){
                ...
                $rethash->{
                    ...
                    'timestamp'} = Date_to_Time($year, $month, $day, $hour, $minute, $second);
            }
            ...
            $rethash->{
                ...
                'timestamp'} = _parse_timestamp($options, $nmeafields[1] . 'h');
            if ($rethash->{
            ...
            if ($nmeafields[10] eq 'M' && $nmeafields[9] = ~ / ^(- ?\d + (|\ . \d +))$ / o){
            ...
            if (@nmeafields >= 6){
                ...
                $rethash->{
                    ...
                    'timestamp'} = _parse_timestamp($options, $nmeafields[5] . 'h');
                if ($rethash->{
    ...
    sub _object_to_decimal($$$$){
        ...
        $rethash->{
            ...
            'timestamp'} = _parse_timestamp($options, $timestamp);
        if ($rethash->{
    ...
    sub _status_parse($$$$){
        ...
        if ($packet = ~ / ^(\d{
            ...
            _a_warn($rethash, 'timestamp_inv_sta') if ($rethash->{
...
sub _comment_telemetry($$){
    ...
    if ($rest = ~ / ^(. *)\|([! -{
                    ...
                    2}
                        ...
                        |)([! -{
                            ...
                            // Forward the location parsing onwards
                            my $locationoffset = 2 + length($rethash->{
                        ...
                        sub _normalpos_to_decimal($$$){
                            ...
                            $rethash->{
                                ...
                                'posresolution'} = _get_posresolution(2 - $rethash->{
                        ...
                        sub _mice_to_decimal($$$$$){
                            ...
                            if ($packet !~ / ^[\x26 - \x7f][\x26 - \x61][\x1c - \x7f]{
                                    2}
                                [\x1c - \x7d][\x1c - \x7f][\x21 - \x7b\x7d][\ / \\A - Z0 - 9] / o){
                                ...
                                if (($options->{
                                            ...
                                            'accept_broken_mice'}
                                        ) && $packet = ~ s / ^([\x26 - \x7f][\x26 - \x61][\x1c - \x7f]{
                                            2}
                                        )\x20([\x21 - \x7b\x7d][\ / \\A - Z0 - 9])(. *) / $1\x20\x20$2$3 / o){
                                    $mice_fixed = 1;
                            ...
                            if ($tmplat = ~ / ^(\d +)(_ *)$ / io){
                                ...
                                $rethash->{
                                    ...
                                    'posresolution'} = _get_posresolution(2 - $amount);
                            }else{
                            ...
                            if ($rethash->{
                                ...
                                _a_err($rethash, 'mice_amb_odd', $rethash->{
					...
					Please note that it's very common to use invalid callsigns on the APRS - IS . = back = cut
                                        ...
                                        _a_err($rh, 'dx_inv_freq') if ($info !~ s / ^(\d + \ . \d +)\s * // );
                                            ...
                                            sub _wx_parse($$){
                                                ...
                                                $w{
                                                    ...
                                                    'wind_gust'} = sprintf('%.1f', $wind_gust * $mph_to_ms) if ($wind_gust = ~ / ^\d + $ /);
                                                $w{

                                                    'wind_direction'} = sprintf('%.0f', $wind_dir) if ($wind_dir = ~ / ^\d + $ /);
                                                $w{

                                                    'wind_speed'} = sprintf('%.1f', $wind_speed * $mph_to_ms) if ($wind_speed = ~ / ^\d + $ /);
                                                $w{

                                                    'temp'} = sprintf('%.1f', _fahrenheit_to_celsius($temp)) if ($temp = ~ / ^ -{
                                                        0, 1}
                                                    \d + $ /);
                                                if ($s = ~ s / r(\d{
                                                            1, 3}
                                                    ...
                                                    $w{
                                                        ...
                                                        'rain_1h'} = sprintf('%.1f', $1 * $hinch_to_mm); # during last 1h
                                                }
                                                if ($s = ~ s / p(\d{
                                                            1, 3}
                                                        )// ) {
                                                    $w{
                                                        ...
                                                        'rain_24h'} = sprintf('%.1f', $1 * $hinch_to_mm); # during last 24h
                                                }
                                                if ($s = ~ s / P(\d{
                                                            1, 3}
                                                        )// ) {
                                                    $w{
                                                        ...
                                                        'rain_midnight'} = sprintf('%.1f', $1 * $hinch_to_mm); # since midnight
                                                }
                                                ...
                                                if ($s = ~ s / h(\d{
                                                    ...
                                                    $w{
                                                        ...
                                                        'humidity'} = sprintf('%.0f', $1); # percentage
                                                    $w{
                                                ...
                                                if ($s = ~ s / b(\d{
                                                    ...
                                                    $w{
                                                        ...
                                                        'pressure'} = sprintf('%.1f', $1 / 10); # results in millibars
                                                }
                                                ...
                                                if ($s = ~ s / ([lL])(\d{
                                                    ...
                                                    $w{
                                                        ...
                                                        'luminosity'} = sprintf('%.0f', $2); # watts / m2
                                                    $w{

                                                        'luminosity'} += 1000 if ($1 eq 'l');
                                                }
                                                ...
                                                if ($s = ~ s / s(\d{
                                                    ...
                                                    $w{
                                                        ...
                                                        'snow_24h'} = sprintf('%.1f', $1 * $hinch_to_mm);
                                                }
                                                ...
                                                $s = ~ s / ^([rPphblLs// ][\. ]{1,5})+//;
                                                    ...
                                                    if ($s = ~ / ^[a - zA - Z0 - 9\ - _]{
                                                        ...
                                                        $w{
                                                            ...
                                                            'soft'} = substr($s, 0, 16) if ($s ne '');
                                                    }else{
                                                        $rh->{

                                                            'comment'} = _cleanup_comment($s);
                                                    }
                                                ...
                                                sub _wx_parse_peet_packet($$$){
                                                    ...
                                                    $w{
                                                        ...
                                                        'wind_gust'} = sprintf('%.1f', $t * $kmh_to_ms / 10) if (defined $t);
                                                    $t = shift @vals;
                                                    $w{

                                                        'wind_direction'} = sprintf('%.0f', ($t&0xff) * 1.41176) if (defined $t); # 1/255 => 1/360
                                                    $t = shift @vals;
                                                    $w{

                                                        'temp'} = sprintf('%.1f', _fahrenheit_to_celsius($t / 10)) if (defined $t); # 1/255 => 1/360
                                                    $t = shift @vals;
                                                    $w{

                                                        'rain_midnight'} = sprintf('%.1f', $t * $hinch_to_mm) if (defined $t);
                                                    $t = shift @vals;
                                                    $w{

                                                        'pressure'} = sprintf('%.1f', $t / 10) if (defined $t && $t >= 10);
                                                    ...
                                                    if (defined $t){
                                                        $w{
                                                            ...
                                                            'humidity'} = sprintf('%.0f', $t / 10); # percentage
                                                        delete $w{
                                                    ...
                                                    $w{
                                                        ...
                                                        'wind_speed'} = sprintf('%.1f', $t * $kmh_to_ms / 10) if (defined $t);
                                                    if (defined $w{
                                                ...
                                                sub _wx_parse_peet_logging($$$){
                                                    ...
                                                    $w{
                                                        ...
                                                        'temp_in'} = sprintf('%.1f', _fahrenheit_to_celsius($t / 10)) if (defined $t); # 1/255 => 1/360
                                                    $t = shift @vals;
                                                    if (defined $t){
                                                        $w{
                                                    ...
                                                    if (defined $t){
                                                        $w{
                                                            ...
                                                            'humidity_in'} = sprintf('%.0f', $t / 10); # percentage
                                                        delete $w{
                                                ...
                                                sub _telemetry_parse($$){
                                                    ...
                                                    // warn 'ok: ' . Dumper(\%t);
                                                    return 1;
                                                } = over = item parseaprs($packet, $hashref, % options)
                                                        ...
                                                        from the Internet (0 - default) .
                                                            ...
                                                            these packets . 1: try to decode, 0: report an error (default) .
                                                                ...
                                                                my $ret = parseaprs("OH2XYZ>APRS,RELAY*,WIDE:!2345.56N/12345.67E-PHG0123 hi",
                                                                    \ % hash, 'isax25' => 0, 'accept_broken_mice' => 0); = back = cut
                                                                ...
                                                                sub parseaprs($$; %){
                                                                    ...
                                                                    if (ord($packettype) == 0x27 || ord($packettype) == 0x60){
                                                                    ...
                                                                    elsif ($packettype eq '!' || $packettype eq '=' || $packettype eq '/' || $packettype eq '@'){
                                                                        ...
                                                                        if ($paclen >= 14){
                                                                            ...
                                                                            if ($packettype eq '/' || $packettype eq '@'){
                                                                                ...
                                                                                $rethash->{
                                                                                    ...
                                                                                    'timestamp'} = _parse_timestamp(\ % options, substr($body, 1, 7));
                                                                                if ($rethash->{
                                                                            ...
                                                                            elsif ($poschar == 47 || $poschar == 92 || ($poschar >= 65 && $poschar <= 90) || ($poschar >= 97 && $poschar <= 106)){
                                                                                // $poschar =~ /^[\/\\A-Za-j]$/o
                                                                                // compressed position
                                                                                if (length($body) >= 13){
                                                                        ...
                                                                        sub _kiss_checkcallsign($){
                                                                            ...
                                                                            // no match
                                                                            return undef;
                                                                        } = over = item kiss_to_tnc2($kissframe)
                                                                        ...
                                                                        sub kiss_to_tnc2($){
                                                                            ...
                                                                            return $asciiframe;
                                                                        } = over = item tnc2_to_kiss($tnc2frame)
                                                                        ...
                                                                        on both ends . If conversion fails, return undef . = back = cut
                                                                        ...
                                                                        sub tnc2_to_kiss($){
                                                                            ...
                                                                            return $kissframe;
                                                                        } = over = item aprs_duplicate_parts($packet)
                                                                        ...
                                                                        In the case of third party packets, always gets this
                                                                            ...
                                                                            (e . g . aprsd replaces trailing CRs or LFs in a packet with a space) . = back = cut
                                                                            ...
                                                                            return undef;
                                                                        } = over = item make_object($name, $tstamp, $lat, $lon, $symbols, $speed, $course, $altitude, $alive, $usecompression, $posambiguity, $comment)
                                                                        ...
                                                                        5th: object symbol table (or overlay) and symbol code, two bytes if the given symbole length is zero (""), use point (// )
                                                                                6th: object speed, - 1 if non - moving (km / h)
                                                                                    ...
                                                                                    9th: alive or dead object (0 == dead, 1 == alive)
                                                                                        ...
                                                                                        parameters should be changed to hash with named parameters . = back = cut
                                                                                        ...
                                                                                        $packetbody .= $comment;
                                                                                        return $packetbody;
                                                                                    } = over = item make_timestamp($timestamp, $format)
                                                                                    ...
                                                                                    Returns a 7 - character string (e . g . "291345z") or undef on error . = back = cut
                                                                                    ...
                                                                                    sub make_timestamp($$){
                                                                                        ...
                                                                                        return $tstring;
                                                                                    } = over = item make_position($lat, $lon, $speed, $course, $altitude, $symbols, $usecompression, $posambiguity)
	...
	8th: use amount (0..4) of position ambiguity. Note that position ambiguity and compression can't be used at the same time .
                                                                                    ...
                                                                                    Python bindings for libfap, L < http:// github.com/kd7lxl/python-libfap> = head1 AUTHORS
                                                                                    Tapio Sokura, OH2KKU E < lt > tapio . sokura@iki . fiE < gt >

                                                                                    Heikki Hannikainen, OH7LZB E < lt > hessu@hes . iki . fiE < gt > = head1 COPYRIGHT AND LICENSE
